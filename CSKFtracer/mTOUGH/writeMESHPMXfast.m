function Gmod=writeMESHPMXfast(infile,outfile,Kmodv,G0)
tic
...
% information about initial file - needed later for check

s0 = dir(infile);

%% write to new MESH file
outfile1=[outfile,'1'];
fid  =  fopen(outfile1,'wt') ;
...
% disp('Copying CONNE...');
% MESHCONN will live in the parent folder so that it can be used by all 
if ~exist('../MESHCONN','file')
...
system(['cat ',outfile1,' ../MESHCONN > ',outfile]);
% opentxt(infile);
% opentxt(outfile);
