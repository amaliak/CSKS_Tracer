...
void SetMetaData(double& L, int& n, doft& dof, int& Ns, int& Nf, int& m, int& level, double& eps) {
    ...
    dof.f   = 1;
    dof.s   = 1;
    Ns      = 1e3;  // Number of sources in simulation cell
    Nf      = 1e3;  // Number of field points in simulation cell
    m       = 1;
    level   = 3;
    eps     = 1e-9;
}
...
void SetSources(vector3 *field, int Nf, vector3 *source, int Ns, double *q, int m,
	...
	for (i=0;i< 10 ;i++ )
	{
		for (j=0;j<10 ;j++ )
		{
			for (k=0;k< 10;k++ )
			{
				source[i*100 + j*10 + k].x = (-1.0 + (2.0/9.0)*((double) i ))*L;
				source[i*100 + j*10 + k].y = (-1.0 + (2.0/9.0)*((double) j ))*L;
				source[i*100 + j*10 + k].z = (-1.0 + (2.0/9.0)*((double) k ))*L;
			}
		}
	}
...
int main(int argc, char *argv[]) {
	...
	cout << "use Chebyshev?" << use_chebyshev << endl;
