...
void kernel_Exp::setHomogen(string& kernelType) {
    homogen = 0;
    symmetry = 1;
    kernelType =  "Exp";
}

void kernel_Exp::EvaluateKernel(vector3 fieldpos, vector3 sourcepos,
                                      double *K, doft *dof){
    vector3 diff;
    ...
    double r = sqrt(diff.x*diff.x/100. + diff.y*diff.y/100. + diff.z*diff.z/4.);
    *K = 0.75*exp(-r);
}
